...
const state = {
    ...
    userName: 'Anonymous',
    isHost: false,
    isSyncing: false, // Prevent circular updates during sync
    lastSyncTime: 0,
    participants: [],
    activities: [],
    messages: []
...
function syncVideoState(videoState) {
    ...
    // Only sync if the update is newer than our last sync and time difference is significant
...
function processFile(file) {
    ...
    const validTypes = ['video/mp4', 'video/x-matroska', 'video/quicktime', 'video/webm', 
...
function renderChatMessages() {
    ...
    state.messages.forEach(msg => {
        ...
        messageEl.innerHTML = `
            <div class="chat-message-header">${msg.userName}</div>
...
function addActivity(description, type = 'info') {
    // Only add if we have a room
    if (!state.roomId) return;
    
    // Activities are now managed server-side through polling
    // This is kept for immediate feedback
    const activity = {
        description,
...
function renderActivities() {
    ...
    elements.activityList.innerHTML = localActivities.map((activity, index) => {
        ...
        return `
            <div class="activity-item">
                ...
                <div class="activity-content ${type}">
                    ...
                    <div class="activity-message">${escapeHtml(activity.description)}</div>
                </div>
            </div>
        `;
    }).join('');
...
function renderActivitiesFromServer() {
    ...
    elements.activityList.innerHTML = activities.map((activity, index) => {
        const type = 'info'; // Server activities don't have types yet
